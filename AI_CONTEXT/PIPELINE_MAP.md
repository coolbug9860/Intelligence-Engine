...
2. **`enrichWithWhiteSpaceDetection`** (`competitorWhitespaceService`) — scrapes 4 publishers (Fortune Business Insights, MarketsandMarkets, Mordor, Allied Market Research). Sets `whiteSpaceStatus`/`Score`/`Competitors`. **Currently only MnM + Mordor return usable data** (FBI returns 0 titles → treated as unreliable; AMR returns HTTP 500). Match threshold 0.40 against report-title AND cleaned-keyword tokens.
