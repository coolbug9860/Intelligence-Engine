...
// Article-preparation logic (relevance gate, retail-noise gate, vertical
// stratification) lives in ./articlePreparation.ts (imported at top of file).
...
Match each signal's weight to what it actually is; never treat a lone procurement notice or patent as if it were an audited SEC disclosure.
...
- reportTitle: Full market research report title following this EXACT formula: [Geographic Modifier] + [Product/Technology Modifier] + [Core Subject] + "Market Size, Share & Forecast, ${FORECAST_RANGE}". Geographic modifier MUST be "Global" unless the signal clearly originates from a specific country or region (e.g. "India", "North America", "Asia Pacific"). Example: "Global AI-Powered Offshore Wind Turbine Predictive Maintenance Market Size, Share & Forecast, ${FORECAST_RANGE}". Never omit the geographic modifier. Never omit the forecast year range. CRITICAL TITLE RULE: The Core Subject must describe ONE single, specific market. Never combine two distinct markets into one title using "and", "&", or "/" — these are separate reports with separate buyers. WRONG: "Global AI Infrastructure and Data Center Market" (two markets). WRONG: "Global Biologics and Cell/Gene Therapy Market" (two markets). RIGHT: "Global AI Data Center Market" OR "Global AI Infrastructure Market" — pick the one the signal most directly supports. If a signal touches two adjacent markets, identify which single market has the stronger buyer demand right now and title the report for that market only. SIGNAL-AS-TRIGGER RULE: The Core Subject is ALWAYS the underlying addressable product, technology, or therapeutic market the signal points to — NEVER the event that surfaced it. M&A deals, funding rounds, patents, regulations, and earnings are TRIGGERS (capture them in the trigger and strategicPillar fields), never the report subject. WRONG: "Global Biotech M&A in Inflammatory & Immune Disease Therapeutics Market…" (an event is not a market). RIGHT: "Global Inflammatory & Immune Disease Therapeutics Market…" with the underlying deal named in the trigger field. LITMUS TEST: if the Core Subject contains "M&A", "acquisition", "merger", "funding", "IPO", "patent", "approval", "policy", or a company name, you have placed the trigger in the subject slot — replace it with the market the event acts upon. GEOGRAPHY-ONCE RULE: Geography appears EXACTLY ONCE, in the leading Geographic Modifier slot. The Core Subject MUST NOT contain any country, region, or nationality word (e.g. "Canadian", "U.S.", "Indian", "European"). If the signal is country-specific, set the Geographic Modifier to that country as a NOUN (e.g. "Canada", not "Canadian") and strip the nationality from the subject. The modifier MUST agree with recommendedReportGeography: if that field is "Country: Canada", the modifier is "Canada" — never "Global". WRONG: "Global Canadian Semiconductor Industry for Defense Applications Market" (two geographies). RIGHT: "Canada Semiconductor for Defense Applications Market Size, Share & Forecast, ${FORECAST_RANGE}". MARKET-NOUN RULE: The Core Subject must name a concrete, addressable market a buyer can purchase syndicated research on — a product, technology, service, material, equipment, or therapeutic category. It must NOT be a cost, price, spend, margin, profitability, or any other financial/operational metric or condition. WRONG: "Global Corn and Soybean Input Costs Market Size, Share & Forecast, ${FORECAST_RANGE}" (input costs are a price dynamic, not a market). RIGHT: "Global Crop Input Market…" or "Global Agricultural Fertilizer Market…" (the underlying product market the cost signal points to). LITMUS: if the Core Subject contains "cost", "costs", "pricing", "price", "prices", "spend", "spending", "margin", "margins", "profitability", or a similar metric word, you have placed a metric in the subject slot — replace it with the product/technology/service market that the metric describes, and capture the cost dynamic in the trigger field instead.
- marketKeyword: SEO keyword phrase following this EXACT formula: [geographic modifier] + [product/technology modifier] + [core subject] + "market". All lowercase. No hyphens. No special characters. This must be the exact phrase a corporate buyer would type into Google. Example: "global ai powered offshore wind turbine predictive maintenance market". Must include the geographic modifier. Must end with "market". Must correspond to ONE searchable market — never combine two markets with "and" in the keyword phrase. The keyword names the MARKET, never the event: it must contain no deal, funding, patent, regulation, or company-name terms — it is the phrase a buyer searches for the market itself, not the news that triggered it. Geography appears once: no nationality adjective inside the phrase. If country-specific, lead with the country noun (e.g. "canada semiconductor for defense applications market"), never "global [nationality] ... market". The keyword must name a real product/technology/service market — never a cost, price, spend, or financial metric: it must not use "costs", "pricing", "prices", "spend", "margins", or "profitability" as the core subject.
...
- rationale: Why this market is actionable now, INTERPRETED BY SIGNAL TYPE: for news/procurement, what changed recently; for patents, why it is a leading indicator and what corroboration exists; for an annual filing (10-K), treat it as lagging confirmation, not a fresh event. 2-3 sentences max.
...
STAT FORMAT RULE:
Every figure must carry an explicit year and projection horizon. Wrong: "The market is growing at 11.4% CAGR." Right: "The market is estimated at roughly 10–13% CAGR through ${forecastEndYear} (directional analyst estimate; key assumption stated)." Do NOT attribute figures to a "Kaiso primary dataset" or any proprietary source not present in the intelligence data above. Either trace a number to a named signal in the data, or label it explicitly as a directional estimate with its assumption.
...
## COMMISSION TITLE
Kaiso report title in this exact format: "Global [Market Name] Market Size, Share & Forecast, ${FORECAST_RANGE}". The market name must be specific and searchable. No "Analysis," no "Study," no em dashes. Example: "Global AI Data Center Cooling Solutions Market Size, Share & Forecast, ${FORECAST_RANGE}". [Market Name] is the underlying product, technology, or therapeutic market — never an event. If the source signal was an M&A deal, funding round, patent, or regulation, that belongs in BURNING PLATFORM / SIGNAL ANALYSIS as the trigger, not in the title. Geography appears EXACTLY ONCE: if this is a country-specific report, lead with the country noun (e.g. "Canada …"), never combine "Global" with a nationality like "Canadian".
...
## MARKET SNAPSHOT
Market size as a clearly-labelled DIRECTIONAL estimate: "USD X.X billion (${currentYear}E, directional), projected to ~USD XX.X billion by ${forecastEndYear} at ~X.X% CAGR — directional estimate based on [named driver/segment from the signals]." If no figure can be grounded in the signal data, give a qualitative range instead and state exactly what primary data would confirm it. Never present an unsourced number as fact, and never attribute it to a Kaiso dataset that is not in the intelligence data above.
